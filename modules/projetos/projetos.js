/**
 * LÓGICA UTILIZADA:
 * Este script é totalmente independente e só é ativado quando o usuário navega para uma das seções de "Projetos".
 * A função setupProjetosEventListeners é o ponto de entrada, chamada pelo scripts.js principal.
 * Ela garante que os listeners de clique para "Novo Orçamento", "Duplicar Linha", etc., só sejam ativados uma vez,
 * evitando bugs de múltiplos cliques. Toda a lógica de manipulação do DOM, cálculos e interações
 * do módulo de projetos está contida aqui.
 */
function setupProjetosEventListeners() {
  const container = document.getElementById("orcamento-lista");
  // Evita adicionar os mesmos listeners múltiplas vezes
  if (!container || container.dataset.listenersAttached === "true") return;

  // Listeners de clique específicos do módulo
  document
